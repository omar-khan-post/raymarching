...
def GetDist(p : vec3) -> float:
    ...
    sphere2 : vec3 = vec3(-2,1,8) #w=1=radius

    sphereDist : float = (p.subtract(sphere)).length - 1
    sphereDist2 : float = (p.subtract(sphere2)).length - 3

    planeDist = p.y #height camera for plane
    d : float = min(sphereDist, planeDist, sphereDist2)
    return d
...
def Circle(uv : vec2 , p : vec2 ,r : float , blur : float ) -> float :

    #create radial gradient
    d :  float  = (uv.subtract(p)).length
    
    #smoothstep(low, high, input)
    c  : float = smoothstep(r, r-blur, d)
...
def mainImage(fragCoord : vec2, iResolution : vec3) -> vec3:
    iTime = time.time()
    p_pixel : vec2 = fragCoord.multiply_by_float(2).subtract(
        iResolution.xy()).divide_by_float(iResolution.y)
    camera_ro_ray_origin : vec3 = vec3(0,0,1)  #x to right, z towards you
    rd_ray_direction : vec3 =  normalize( vec3(p_pixel.x, p_pixel.y, -1.5))#looking at origin backwards

#47.33

    #gradient base sky using y component of vector
    color_pixel : vec3= vec3(0.65, 0.75, 0.9).subtract_by_float(0.5 *rd_ray_direction.y )#blue sky if no hits
...
def main():
    ...
    for j in range( (ny-1), -1, -1): #j=ny-1;j>=0;j--
        for i in range( 0, nx, 1): #i=0;i<nx;i++
            ...
            #)
            #vec3_col = color(r)
            fragColor = mainImage( fragCoord,iResolution)
    ...
    #print(file_content)
    ppm = open("ray-march-bounce-yellow-light.ppm", "w")
